        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.875rem" }}>
          ...
          <div>
            ...
            <p style={{ color: "var(--color-neutral-500)", fontSize: "0.875rem", marginTop: "0.125rem" }}>
              Your properties only · Tenants are notified by email.
            </p>
          </div>
        </div>
        <button onClick={() => { setShowForm(!showForm); setFormError(""); setTitle(""); setMessage(""); setPropertyId(""); }}
          style={{ background: showForm ? "#f3f4f6" : "#111827", color: showForm ? "#374151" : "#ffffff", border: "none", borderRadius: "8px", padding: "0.625rem 1.25rem", fontWeight: "600", fontSize: "0.875rem", cursor: "pointer" }}>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem", marginBottom: "1rem" }}>
              ...
              <div>
                ...
                <input value={title} onChange={e => { setTitle(e.target.value); setFormError(""); }}
                  placeholder="e.g. Water supply maintenance on Friday" style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Message *</label>
                <textarea value={message} onChange={e => { setMessage(e.target.value); setFormError(""); }}
                  placeholder="Write your notice here..." rows={5} style={{ ...inputStyle, resize: "vertical" }} />
              </div>
            </div>
            <div style={{ display: "flex", gap: "0.75rem", justifyContent: "flex-end" }}>
              <button type="button" onClick={() => { setShowForm(false); setTitle(""); setMessage(""); setPropertyId(""); }}
                style={{ ...actionBtnStyle, background: "#f3f4f6", color: "#374151" }}>Cancel</button>
              <button type="submit" disabled={submitting}
                style={{ ...actionBtnStyle, background: "#111827", color: "#ffffff", opacity: submitting ? 0.7 : 1 }}>
      ...
      {loading ? <LoadingSpinner /> : error ? <ErrorState message={error} /> :
        notices.length === 0 ? (
          <div className="card" style={{ textAlign: "center", padding: "3rem", color: "var(--color-neutral-400)" }}>
            No notices posted yet. Click <strong>+ Post Notice</strong> to create one.
          </div>
        ) : (
            ...
            {notices.map(n => (
              <div key={n._id} className="card">
                <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "0.5rem", marginBottom: "0.5rem" }}>
                  <p style={{ fontWeight: "600", color: "#111827", margin: 0 }}>{n.title}</p>
                  <div style={{ display: "flex", gap: "0.75rem", alignItems: "center" }}>
                    <span style={{ fontSize: "0.75rem", color: "var(--color-neutral-400)" }}>{fmtDate(n.createdAt)}</span>
                    <span style={tagStyle}>{n.propertyId?.title ?? "—"}</span>
                  </div>
                </div>
                <p style={{ fontSize: "0.875rem", color: "var(--color-neutral-700)", lineHeight: "1.6", whiteSpace: "pre-wrap", margin: 0 }}>{n.message}</p>
              </div>
            ))}
          </div>
        )
...
export function AdminNoticeSection({ onBack }) {
  ...
  const [title,      setTitle]      = useState("");
  const [message,    setMessage]    = useState("");
  const [propertyId, setPropertyId] = useState(""); // empty = global
  const [properties, setProperties] = useState([]);
  const [submitting, setSubmitting] = useState(false);
  const [formError,  setFormError]  = useState("");
  const [success,    setSuccess]    = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(propertyId
        ? "Notice posted to property and tenants notified by email!"
        : "Platform-wide notice posted and all users notified by email!");
      setTitle(""); setMessage(""); setPropertyId(""); setShowForm(false);
      fetchNotices(); setTimeout(() => setSuccess(""), 5000);
    } catch (err) { setFormError(err.message); } finally { setSubmitting(false); }
  };
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1.5rem", flexWrap: "wrap", gap: "1rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.875rem" }}>
          ...
          <div>
            <h3 style={{ margin: 0 }}>Notices</h3>
            <p style={{ color: "var(--color-neutral-500)", fontSize: "0.875rem", marginTop: "0.125rem" }}>
              All properties · Leave property blank for a platform-wide notice.
            </p>
          </div>
        </div>
        <button onClick={() => { setShowForm(!showForm); setFormError(""); setTitle(""); setMessage(""); setPropertyId(""); }}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem", marginBottom: "1rem" }}>
              <div>
                <label style={labelStyle}>
                  Property <span style={{ color: "#9ca3af", fontWeight: "400" }}>(leave blank for platform-wide)</span>
                </label>
                <select value={propertyId} onChange={e => setPropertyId(e.target.value)} style={inputStyle}>
              ...
              <div>
                <label style={labelStyle}>Title *</label>
                <input value={title} onChange={e => { setTitle(e.target.value); setFormError(""); }}
                  placeholder="e.g. Scheduled maintenance downtime" style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Message *</label>
                <textarea value={message} onChange={e => { setMessage(e.target.value); setFormError(""); }}
            ...
            {notices.map(n => (
              <div key={n._id} className="card">
                <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "0.5rem", marginBottom: "0.5rem" }}>
                  ...
                  <div style={{ display: "flex", gap: "0.75rem", alignItems: "center" }}>
                    <span style={{ fontSize: "0.75rem", color: "var(--color-neutral-400)" }}>{fmtDate(n.createdAt)}</span>
                    <span style={{ ...tagStyle, background: n.isGlobal ? "#fef3c7" : "var(--color-info-50)", color: n.isGlobal ? "#b45309" : "var(--color-info-700)" }}>
                      {n.isGlobal ? "Platform-wide" : (n.propertyId?.title ?? "—")}
                    </span>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.875rem" }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "0.625rem" }}>
              <h3 style={{ margin: 0 }}>Notices</h3>
              {unreadCount > 0 && (
                <span style={{ background: "#dc2626", color: "#ffffff", borderRadius: "999px", fontSize: "0.7rem", fontWeight: "700", padding: "0.1rem 0.5rem", minWidth: "18px", textAlign: "center" }}>
                  {unreadCount}
                </span>
              )}
            </div>
            <p style={{ color: "var(--color-neutral-500)", fontSize: "0.875rem", marginTop: "0.125rem" }}>
              Notices from your property owner and platform admin.
            </p>
          </div>
        </div>
        {unreadCount > 0 && (
          <button onClick={handleMarkAll}
            style={{ background: "#f3f4f6", border: "none", borderRadius: "8px", padding: "0.5rem 1rem", fontSize: "0.8125rem", fontWeight: "600", color: "#374151", cursor: "pointer" }}>
            Mark all as read
          </button>
        )}
      </div>
            ...
            {notices.map(n => {
              ...
              return (
                <div key={n._id} className="card" onClick={() => handleOpen(n._id, n.isRead)}
                  ...
                  <p style={{ fontSize: "0.875rem", color: isUnread ? "#374151" : "var(--color-neutral-600)", fontWeight: isUnread ? "500" : "400", lineHeight: "1.6", whiteSpace: "pre-wrap", margin: 0 }}>
                    {n.message}
                  </p>
                </div>
              );
...
const actionBtnStyle   = { padding: "0.625rem 1.25rem", borderRadius: "8px", border: "none", fontWeight: "600", fontSize: "0.875rem", cursor: "pointer" };
...
const tagStyle         = { fontSize: "0.75rem", background: "var(--color-info-50)", color: "var(--color-info-700)", padding: "0.15rem 0.5rem", borderRadius: "9999px" };