...
import { getUser, adminAPI, maintenanceAPI } from "../../../lib/api";
...
// ── Technician defaults per category ──────────────────────
...
// ── Dropdown options for technician assignment ────────────
            ...
            <div className="card">
              ...
              {mLoading ? <LoadingSpinner /> : mError ? <ErrorState message={mError} /> : (
                <DataTable
                  ...
                  rows={mRequests.slice(0, 5).map(r => [
              ...
              {users.loading ? <LoadingSpinner /> : users.error ? <ErrorState message={users.error} /> : (
                <DataTable
                  ...
                  rows={(users.data?.users ?? []).map(u => [
                    u.name, u.email, u.role, u.university ?? "—", u.phone ?? "—", fmtDate(u.createdAt),
                  ])}
                />
              )}
            </div>
          ...
          <div>
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              <p style={{ color: "var(--color-neutral-500)", fontSize: "0.875rem", marginTop: "0.25rem" }}>
                All properties · Assign technicians and update statuses.
              </p>
            </div>
