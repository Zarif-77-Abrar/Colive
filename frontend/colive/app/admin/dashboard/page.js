...
export default function AdminDashboard() {
  ...
  const mFiltered = mRequests
    ...
    .filter(r => {
      ...
      return (r.propertyId?.title ?? "").toLowerCase().includes(q) || (r.propertyId?._id ?? "").toLowerCase().includes(q);
    });
  ...
  const gFiltered = gLogs
    ...
    .filter(g => {
      ...
      return (g.propertyId?.title ?? "").toLowerCase().includes(q) || (g.propertyId?._id ?? "").toLowerCase().includes(q);
    });
      ...
      <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "2rem 1.5rem" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "1rem" }}>
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: "0.625rem", marginBottom: "0.375rem" }}>
                          ...
                          <span style={{ fontSize: "0.7rem", fontWeight: "600", padding: "0.15rem 0.5rem", borderRadius: "9999px", background: u.role === "admin" ? "var(--color-error-50)" : u.role === "owner" ? "var(--color-warning-50)" : "var(--color-primary-50)", color: u.role === "admin" ? "var(--color-error-700)" : u.role === "owner" ? "var(--color-warning-700)" : "var(--color-primary-700)" }}>
                            {u.role}
                          </span>
                          {u.isBlacklisted && (
                            <span style={{ fontSize: "0.7rem", fontWeight: "600", padding: "0.15rem 0.5rem", borderRadius: "9999px", background: "var(--color-error-50)", color: "var(--color-error-700)", border: "1px solid var(--color-error-300)" }}>
                              Blacklisted
                            </span>
                          )}
                        </div>
                        <p style={{ fontSize: "0.875rem", color: "var(--color-neutral-500)", marginBottom: "0.25rem" }}>{u.email} {u.university ? `· ${u.university}` : ""}</p>
                        <p style={{ fontSize: "0.8125rem", color: "var(--color-neutral-400)" }}>Joined {fmtDate(u.createdAt)}</p>
                        {u.isBlacklisted && u.blacklistReason && (
                          <p style={{ fontSize: "0.8125rem", color: "var(--color-error-600)", marginTop: "0.375rem" }}>Reason: {u.blacklistReason}</p>
                        )}
                      </div>
                      {u.role !== "admin" && (
        ...
        {active === "maintenance" && (
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {mLoading ? <LoadingSpinner /> : mError ? <ErrorState message={mError} /> :
                mFiltered.length === 0
                  ? <div className="card" style={{ textAlign: "center", padding: "3rem", color: "var(--color-neutral-400)" }}>No requests found.</div>
                  : mFiltered.map(r => (
        ...
        {active === "notices" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
              <h3>All notices</h3>
              <button className="btn btn-primary btn-sm" onClick={() => { setActive("others"); setOthersSection("notice"); }}>
                + Post notice
              </button>
            </div>
        ...
        {active === "others" && (
          <div>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                  {gLoading ? <LoadingSpinner /> : gError ? <ErrorState message={gError} /> :
                    gFiltered.length === 0
                      ? <div className="card" style={{ textAlign: "center", padding: "3rem", color: "var(--color-neutral-400)" }}>No guest entries found.</div>
                      : gFiltered.map(g => (
                            ...
                            <select
                              ...
                              onChange={async (e) => {
                                try {
                                  ...
                                  setGSuccess("Status updated."); fetchGuestLogs();
            ...
            {othersSection === "meal"   && <AdminMealSection   onBack={() => setOthersSection(null)} />}
