      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "2rem 1.5rem" }}>
        ...
        {active === "bookings" && (
          <div>
            ...
            <div className="card">
              {bookings.loading ? <LoadingSpinner /> : bookings.error ? <ErrorState message={bookings.error} /> : (
                <DataTable headers={["Tenant", "Property", "Room", "Rent", "Compatibility", "Status", "Date"]} emptyMessage="No booking requests received yet."
                  rows={(bookings.data?.bookings ?? []).map(b => [
                    b.tenantId?.name ?? "—", b.propertyId?.title ?? "—", b.roomId?.label ?? "—", fmtMoney(b.roomId?.rent),
