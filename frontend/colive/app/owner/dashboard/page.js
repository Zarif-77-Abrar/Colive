                    ...
                    <div key={b._id} style={{ padding: "1rem", borderRadius: "var(--radius-md)", border: "1px solid var(--color-neutral-200)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
                      <div>
                        ...
                        <p style={{ fontSize: "0.8125rem", color: "var(--color-neutral-500)" }}>
                          {fmtMoney(b.roomId?.rent)}/mo{b.compatibilityScore != null ? ` · ${b.compatibilityScore}% compatibility` : ""} · {fmtDate(b.createdAt)}
                        </p>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "0.625rem" }}>
                        {b.status === "pending" ? (
                          <>
                            <button className="btn btn-primary btn-sm" disabled={!!actionLoading} onClick={() => handleBookingAction(b._id, "accept")}>
                              {actionLoading === b._id + "accept" ? "..." : "Accept"}
                            </button>
                            <button className="btn btn-danger btn-sm" disabled={!!actionLoading} onClick={() => handleBookingAction(b._id, "reject")}>
                              {actionLoading === b._id + "reject" ? "..." : "Decline"}
                            </button>
                          </>
                        ) : (
                          <span style={{ fontSize: "0.75rem", fontWeight: "500", padding: "0.2rem 0.625rem", borderRadius: "9999px", background: b.status === "accepted" ? "var(--color-success-50)" : "var(--color-error-50)", color: b.status === "accepted" ? "var(--color-success-700)" : "var(--color-error-700)" }}>
                            {b.status}
                          </span>
                        )}
                      </div>
        ...
        {active === "maintenance" && (
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {mLoading ? <LoadingSpinner /> : mError ? <ErrorState message={mError} /> :
                mFiltered.length === 0
                  ? <div className="card" style={{ textAlign: "center", padding: "3rem", color: "var(--color-neutral-400)" }}>No requests in this category.</div>
                  : mFiltered.map(r => (
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                  {gLoading ? <LoadingSpinner /> : gError ? <ErrorState message={gError} /> :
                    gFiltered.length === 0
                      ? <div className="card" style={{ textAlign: "center", padding: "3rem", color: "var(--color-neutral-400)" }}>No guest entries in this category.</div>
                      : gFiltered.map(g => (
