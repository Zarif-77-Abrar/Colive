...
export const acceptBooking = async (req, res) => {
  try {
    ...
    // Create an initial pending payment for the current month
    try {
      const now = new Date();
      ...
      console.error("Failed to create initial payment:", paymentErr.message);
    }
...
// ── PUT /api/bookings/:id/leave ────────────────────────────
export const leaveBooking = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Ensure only the tenant who owns the booking can leave
    if (booking.tenantId.toString() !== req.user.id) {
      return res.status(403).json({ message: "Unauthorized." });
    }
    // Ensure the booking is actually currently accepted
    if (booking.status !== "accepted") {
      return res.status(400).json({ message: "You can only leave a room you are currently occupying." });
    }

    // Block leaving if current month's rent is unpaid
    const now = new Date();
    const currentMonth = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}`;
    ...
    if (!currentMonthPayment || currentMonthPayment.paymentStatus !== "paid") {
      return res.status(403).json({
        message: `You must pay your rent for ${currentMonth} before leaving. Please clear your dues first.`,
      });
    }
